...
int disk_read(int block, char *buf)
{
		...
		memcpy(buf, disk[block], BLOCK_SIZE); //copy BLOCK_SIZE amount of data in disk[block] to buff array
...
int disk_write(int block, char *buf)
{
		...
		memcpy(disk[block], buf, BLOCK_SIZE); //copy BLOCK_SIZE amount of data in buff array to disk[block]
